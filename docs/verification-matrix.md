...
| compose golden path + repo-url-mode MCP drive | `python3 scripts/e2e_golden_path.py` | Compose E2E | yes | pass | Exit 0. Both `MCP stdio bridge passed` (pinned mode) and `MCP repo-url mode passed` (second `drive_mcp_stdio` pass: third config dir with no `project_id`, cwd inside a temp repo whose `origin` matches the bootstrap project's `repository_url`; all six tools succeeded, distinct `current_version` pair, feedback `refuted` applied with `already_applied=False`). First run failed on a test-design bug (reused the pinned pass's already-stale memory -> `memory_not_editable`); fixed by giving the repo-url pass its own hook-created memory. Local run excludes the frontend image (pre-existing pnpm/proxy env blocker; CI builds the full stack). |
