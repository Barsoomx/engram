...
class BackendRuntimeLayoutTests(unittest.TestCase):
    ...
    def test_backend_runtime_layout_does_not_require_manual_observation_outbox_command(
        self,
    ) -> None:
        self.assertNotIn(
    ...
    def test_import_app_is_installed(self) -> None:
        settings = (ROOT / "apps/backend/settings/settings.py").read_text(
            encoding="utf-8"
        )
    ...
    def test_celery_foundation_uses_sla_queues_and_confirm_publish(self) -> None:
        celeryconfig = (ROOT / "apps/backend/engram/celeryconfig.py").read_text(
            encoding="utf-8"
        )
        celery_app = (ROOT / "apps/backend/engram/celery_app.py").read_text(
            encoding="utf-8"
        )
        settings = (ROOT / "apps/backend/settings/settings.py").read_text(
            encoding="utf-8"
        ...
        self.assertIn(
            "task_cls='engram.core.retryable_django_task.RetryableTask'", celery_app
        )
    ...
    def test_claude_mem_fixture_is_text_reviewable_and_sanitized(self) -> None:
        ...
        fixture_text = "\n".join(
            path.read_text(encoding="utf-8") for path in fixture_files
        )
...
class BackendComposeContractTests(unittest.TestCase):
    ...
    def test_compose_relay_is_package_transport_not_domain_outbox_processing(
        self,
    ) -> None:
        compose = (ROOT / "deploy/compose/docker-compose.yml").read_text(
            encoding="utf-8"
    ...
    def test_compose_e2e_workflow_runs_golden_path_script(self) -> None:
        workflow = (ROOT / ".github/workflows/compose-e2e.yml").read_text(
            encoding="utf-8"
        )
    ...
    def test_golden_path_waits_for_relayed_tasks_instead_of_manual_outbox_processing(
        self,
    ) -> None:
    ...
    def test_golden_path_proves_current_hook_observation_created_context_memory(
        self,
    ) -> None:
        script = (ROOT / "scripts/e2e_golden_path.py").read_text(encoding="utf-8")
        ...
        self.assertIn(
            "str(version.source_observation_id) not in document.source_observation_ids",
            script,
        )
