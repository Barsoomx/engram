...
MEMORY_BODY_PREFIX = (
    "The hook ingest path reuses accepted replay rows and keeps request ids idempotent."
)
...
def main() -> int:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="engram-e2e-") as config_dir:
            ...
            assert_equal(
                post_tool_use.get("status"), "accepted", "post-tool-use status"
            )
            assert_secret_absent(
                "post-tool-use response", json.dumps(post_tool_use), api_key
            )
            ...
            retrieval_document_id = required_string(
                worker_memory, "retrieval_document_id"
            )
...
def wait_for_worker_memory(
    project_id: str, run_id: str, secret: str
) -> dict[str, object]:
...
def context_audit_query(
    context_bundle_id: str, retrieval_document_id: str, request_id: str
) -> str:
    return f"""
import json
...
def run_json(
    ...
    *,
    ...
    try:
        ...
        raise E2EError(
            f"Expected JSON output from {redact(' '.join(args), secret)}"
        ) from error
...
def assert_context_response(
    response: dict[str, object], worker_memory: dict[str, object]
) -> None:
