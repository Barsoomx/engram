...
def generation_content(system_prompt: str, prompt: str) -> str:
    ...
    if 'single structured observation' in system_prompt:
        return json.dumps(
            {
                'memories': [
                    {
                        'title': f'{GENERATION_TITLE_PREFIX} {digest_suffix}',
                        'body': f'Fixed durable engineering memory produced by the mock provider for {digest_suffix}.',
                        'confidence': 0.9,
                    },
                ],
            },
