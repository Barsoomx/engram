...
the thin `engram` CLI. It ships no local worker, no local database, and no
...
Install the plugin from a Claude Code plugin marketplace (see
`plugin-repository/README.md` for the marketplace manifest format):

```bash
claude plugin install engram@<marketplace-name>
```
...
The plugin registers the following Claude Code hook events. Each event calls
`engram hook <event> --agent claude_code --response-format claude-code`.

| Event         | Matcher            | Command                                                                                  | Timeout (s) |
| ------------- | ------------------ | ---------------------------------------------------------------------------------------- | ----------- |
| `SessionStart` | `startup\|resume`  | `engram hook session-start --agent claude_code --response-format claude-code`            | 60          |
...
2. Reads the corresponding `~/.engram/hooks.claude_code.json` manifest produced
   by `engram connect`.
3. POSTs the hook payload to the Engram server
   (`/v1/hooks/<event>`, and `/v1/context/session-start` for `SessionStart`).
4. Prints the server response in the `claude-code` response format on stdout,
