...
class McpContractTests(unittest.TestCase):
    ...
    def test_tools_list_descriptions_direct_proactive_search(self) -> None:
        ...
        self.assertEqual(
            ...
            "Step 1 - ALWAYS search project memory BEFORE starting any non-trivial task (bug fix, feature, refactor, debugging, code review) and BEFORE asserting how a subsystem works. Returns prior decisions, gotchas, incidents and architecture notes ranked by relevance — 30 seconds here routinely saves re-deriving a root cause someone already found. Call it when the user references past work ('did we', 'last time', 'as before', 'мы уже делали', 'в прошлый раз', 'как раньше', 'как в тот раз'), names a subsystem, or reports an error you have not seen this session. Prefer short 2-4 word queries (symptom, component, error text), in the language the memories are written in (mostly English here); run 2-3 differently-angled queries rather than one broad one. Filter by kinds=[convention,decision] to fetch project conventions or decisions on a topic (e.g. gitlab workflow).",
        )
        self.assertEqual(
            descriptions["engram_observations"],
            "Step 2 - list recent raw observations (prompts, tool activity, hook events) captured for the connected project. Use when you need ground truth about a past or interrupted session: the exact command that failed, the exact error text, what an earlier agent actually did — or to corroborate a memory found via engram_search before relying on it. Time filters since/until bound ingestion time (created_at, until exclusive); results still display and sort by observed_at.",
        )
        self.assertEqual(
            descriptions["engram_context"],
            "Re-request the memory context bundle that is injected at session start (recent and relevant approved memories for this project). Call it FIRST THING after /clear, context compaction, or resuming a continued session — the injected context is gone and you are working without your project memory until you re-fetch it. Also use it when the injected Engram context looks stale or unrelated to the current task. Filter by kinds=[convention,decision] to fetch project conventions or decisions on a topic (e.g. gitlab workflow).",
        )
        self.assertEqual(
            descriptions["engram_memory_feedback"],
            "Step 3 - close the loop on every memory you actually used. The moment you discover an injected or retrieved memory is outdated or wrong, mark it stale or refuted with a reason. When you acted on a memory and it proved accurate, confirm it — confirmation resets its confidence decay clock and keeps it surfacing. Clean memory improves every future session: do not silently ignore bad memory, and do not let good memory decay unconfirmed.",
        )
        self.assertEqual(
            descriptions["engram_memory_link"],
            "Attach a file/symbol/commit/issue link to an approved memory so future sessions surface it when touching that exact file or symbol — linked memories are retrieved by path/symbol match that plain text search misses. Use it the moment you identify WHERE a memory's fact lives in code: after fixing a bug a memory described, after verifying which module implements a documented behavior, or after moving code a memory references.",
        )
        self.assertEqual(
            descriptions["engram_memory_version"],
            "Update an approved memory body, creating a new reviewed version. Use when a memory is right in substance but wrong or incomplete in detail — line numbers moved, a flag was renamed, you verified a materially better explanation. Prefer this over refuting and re-proposing: it preserves history, links, and confidence. Read the full body with engram_memory_get first so the new version loses nothing.",
        )
    ...
    def test_tools_list_read_tool_descriptions_are_verbatim_and_unnumbered(self) -> None:
        ...
        self.assertEqual(
            ...
            "Read one memory in full by memory_id — the complete untruncated current body, version history, and links, not the 400-char session-start preview. Use whenever a search or context snippet is cut off mid-thought, before quoting a memory as authority, and ALWAYS before revising, linking, or giving feedback so you act on the full stored text. Kind, confidence, and conflict/stale/refuted validity come from engram_search, not this tool.",
        )
        self.assertEqual(
            descriptions["engram_audit"],
            "Answer 'why is this memory in this state and what changed it' — every transition committed against a memory (promotion, revise, refute, stale, restore, supersede, archive, a candidate merged into it, and a merge where it is the source), most recent first. Without memory_id it lists project-wide events — useful to see what curation did recently. With an org-wide key you must pass project_id explicitly. Not returned: the winner side of a supersession (a direct merge is recorded under the source memory; a candidate supersession that creates a new winner is recorded under the superseded loser), confidence-decay, and link add/remove events — those are keyed to a different audit target.",
        )
