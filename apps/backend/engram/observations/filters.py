...
class ObservationFilterSet(SinceUntilFilterSet):
    team_id = django_filters.UUIDFilter(method='filter_team_id')
    ...
    def filter_team_id(self, queryset: QuerySet, name: str, value: uuid.UUID) -> QuerySet:
        return queryset.filter(Q(team_id__isnull=True) | Q(team_id=value))
