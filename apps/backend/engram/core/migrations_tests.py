...
MIGRATE_0041 = [('core', '0041_agent_proposal_source')]
MIGRATION_0041_NODE = ('core', '0041_agent_proposal_source')
MIGRATION_0041_MODULE = 'engram.core.migrations.0041_agent_proposal_source'
...
def test_0041_reverse_guard_is_last_and_refuses_agent_proposals() -> None:
    migration_module = importlib.import_module(MIGRATION_0041_MODULE)
    migration = migration_module.Migration
    operation = migration.operations[-1]
    assert operation.__class__.__name__ == 'RunPython'
    ...
    assert operation.reverse_code is not None
    with pytest.raises(RuntimeError, match='cannot reverse 0041'):
        operation.reverse_code(_Apps(), None)


@pytest.mark.django_db(transaction=True)
def test_0041_forward_preserves_rows_and_reverse_guards_agent_proposal() -> None:
    executor = MigrationExecutor(connection)
    leaf_nodes = executor.loader.graph.leaf_nodes()
    assert MIGRATION_0041_NODE in executor.loader.graph.nodes
    try:
        executor.migrate(MIGRATE_0040)
        old_apps = executor.loader.project_state(MIGRATE_0040).apps
        scope = _create_historical_0032b_scope(old_apps)
        session = _create_historical_session(old_apps, scope, 'migration-0041-session')
        observation = _create_historical_observation(
            old_apps,
            scope,
            session,
        ...
        _stage_model, stage = _create_historical_0036_stage_fixture(old_apps, '0041')
        source_model = old_apps.get_model('core', 'MemoryCandidateSource')
        ...
        observation_source = old_apps.get_model('core', 'ObservationSource').objects.create(
            ...
            observation=observation,
            source_type='claude_mem',
            source_id='migration-0041:import',
        )
        ...
        executor = MigrationExecutor(connection)
        executor.migrate(MIGRATE_0041)
        new_apps = executor.loader.project_state(MIGRATE_0041).apps
        ...
        agent_source.delete()
        MigrationExecutor(connection).migrate(MIGRATE_0040)
        reverted_apps = MigrationExecutor(connection).loader.project_state(MIGRATE_0040).apps
